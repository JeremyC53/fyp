...
# Pitch Analysis
#########################################
def pitch_analysis(ref_file, user_rep_files, user_colors):
    ...
    if len(time_ref) > 1:
        ...
        avg_speed_ref = 0
    print(f"Reference Rep: Range of Motion = {rom_ref:.2f}°, Average Speed = {avg_speed_ref:.2f}°/s")
    
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    axes[0].set_title("Original Pitch Angle Over Time (User Reps vs Reference Rep)")
    axes[0].set_xlabel("Timestamp (s)")
    axes[0].set_ylabel("Pitch Angle (°)")
    axes[0].grid(True)
    axes[0].plot(df_ref["timestamp"], df_ref["pitch"], label="Reference Rep", color='black')

    axes[1].set_title("Time-Shifted User Reps Compared to Reference Rep")
    axes[1].set_xlabel("Timestamp (s)")
    axes[1].set_ylabel("Pitch Angle (°)")
    axes[1].grid(True)
    axes[1].plot(df_ref["timestamp"], df_ref["pitch"], label="Reference Rep", color='black')
    ...
    for i, user_file in enumerate(user_rep_files):
        ...
        if len(time_user) > 1:
            ...
            avg_speed_user = 0
        print(f"User Rep {i+1}: Range of Motion = {rom_user:.2f}°, Average Speed = {avg_speed_user:.2f}°/s")
        time_shift = trough_time_ref - trough_time_user
        df_user["shifted_timestamp"] = df_user["timestamp"] + time_shift
        axes[0].plot(df_user["timestamp"], df_user["pitch"], linestyle='dashed',
                     color=user_colors[i], label=f"User Rep {i+1}")
        axes[1].plot(df_user["shifted_timestamp"], df_user["pitch"], linestyle='dashed',
...
def compare_motions(ref_positions, user_positions, threshold=0.2):
    ...
    prompts = []
    if max_diff[1] > threshold:
        prompts.append("Please tuck your elbow in — it’s flaring too much. Keep your elbow aligned.")
    if max_diff[0] > threshold:
        prompts.append("Keep your arm closer to your body.")
    if max_diff[2] > threshold:
        prompts.append("You are not completing the full bicep curl motion. Please perform the complete motion.")
    if not prompts:
...
def compare_peak_speed(ref_ang_speeds, user_ang_speeds, tolerance=0.5):
    ref_peak = np.max(ref_ang_speeds)
    user_peak = np.max(user_ang_speeds)
    print(f"Reference Peak Angular Speed: {ref_peak:.2f} rad/s")
    print(f"User Peak Angular Speed: {user_peak:.2f} rad/s")
    if user_peak > ref_peak + tolerance:
        return "Your angular speed is too high; please control your speed!"
    elif user_peak < ref_peak - tolerance:
        return "Your angular speed is too low; try to increase your speed."
    else:
        return "Good speed! Your motion is well controlled."
...
def compare_stability(ref_ang_speeds, user_ang_speeds, tolerance_cv=0.2):
    ...
    if user_cv > ref_cv + tolerance_cv:
        return "Your motion appears unstable. Try to maintain a smoother, more controlled speed."
    else:
...
def dtw_analysis(ref_file, user_rep_files, user_colors, local_offset=np.array([0.0, 0.0, 0.3])):
    ...
    for i, user_file in enumerate(user_rep_files):
        ...
        if avg_adjusted_dist < 3:
            print("Good job! Your overall motion is immaculate.")
        else:
            print("There are areas for improvement in your motion.")
...
def angular_speed_analysis(ref_file, user_rep_files, user_colors):
    ...
    for i, user_file in enumerate(user_rep_files):
        ...
        user_ang_speeds_last = user_ang  # keep the last rep's speeds
    plt.xlabel("Time (s)")
    ...
    plt.tight_layout()
    plt.show()
    if user_ang_speeds_last is not None:
        feedback = compare_peak_speed(ref_ang_speeds, user_ang_speeds_last, tolerance=0.5)
        print(feedback)
...
#########################################
def main(folder="motion_files", duration=5.04, port='COM4'):
    """
    Runs the full bicep curl analysis workflow:
    ...
    pitch_analysis(ref_file, user_rep_files, user_colors)
