...
def motion_analysis(ref_file, user_rep_files, user_colors):
    ...
    # ----- PITCH PLOTS -----
    # Original pitch
    axes[0, 0].set_title("Original Pitch Angle Over Time\n(User Reps vs Reference Rep)")
    ...
    # ----- ROLL PLOTS -----
    # Original roll
    axes[1, 0].set_title("Original Roll Angle Over Time\n(User Reps vs Reference Rep)")
    ...
    # ----- YAW PLOTS -----
    # Original yaw
    axes[2, 0].set_title("Original Yaw Angle Over Time\n(User Reps vs Reference Rep)")
...
def compare_motions(ref_positions, user_positions, threshold=0.2):
    ...
    if max_diff[0] > 0.03:
        # Find the index of the maximum absolute difference for coordinate 0
        idx = np.argmax(np.abs(ref_positions[:, 0] - user_positions[:, 0]))
        # Compare the reference and user positions at that index
        if ref_positions[idx, 0] > user_positions[idx, 0]:
            prompts.append("Please keep your arm closer to the body by rotating your arm anticlockwise.")
        else:
            prompts.append("Please keep your arm closer to the body by rotating your arm clockwise.")
...
#########################################
def main(folder="srs", duration=5, port='COM4'):
    """
