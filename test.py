...
# Global list to collect all feedback messages
all_feedback = []

#########################################
# OpenAI Summarisation Function
#########################################
def summarise_feedback(feedback_list):
    combined_feedback = "\n".join(feedback_list)
    prompt = (
        "Summarise the following feedback for all the reps into one concise line of feedback:\n\n"
        f"{combined_feedback}"
    )
...
def motion_analysis(ref_file, user_rep_files, user_colors):
    ...
    if len(time_ref) > 1:
        ...
        avg_speed_ref = 0
    ref_msg = f"Reference Rep: Range of Motion (Pitch) = {rom_ref:.2f}°, Average Speed = {avg_speed_ref:.2f}°/s"
    print(ref_msg)
    all_feedback.append(ref_msg)
    ...
    for i, user_file in enumerate(user_rep_files):
        ...
        if len(time_user) > 1:
            ...
            avg_speed_user = 0
        user_msg = f"User Rep {i+1}: Range of Motion (Pitch) = {rom_user:.2f}°, Average Speed = {avg_speed_user:.2f}°/s"
        print(user_msg)
        all_feedback.append(user_msg)
...
def compare_motions(ref_positions, user_positions, threshold=0.2):
    ...
    if max_diff[0] > 0.15:
        prompts.append("Please perform the full range of motion without exceeding nor falling short of range.")
...
def compare_peak_speed(ref_ang_speeds, user_ang_speeds_list, tolerance=0.5):
    ...
    for i, user_ang_speeds in enumerate(user_ang_speeds_list):
        ...
        msg = f"User Rep {i+1} Peak Angular Speed: {user_peak:.2f} rad/s"
        print(msg)
        messages.append(msg)
        if user_peak > ref_peak + tolerance:
            feedback = f"User Rep {i+1}: Your angular speed is too high; please control your speed!"
        elif user_peak < ref_peak - tolerance:
            feedback = f"User Rep {i+1}: Your angular speed is too low; try to increase your speed."
        else:
            feedback = f"User Rep {i+1}: Good speed! Your motion is well controlled."
        print(feedback)
...
def compare_stability(ref_ang_speeds, user_ang_speeds, tolerance_cv=0.2):
    ...
    msg1 = f"Reference Angular Speed: Mean = {ref_mean:.2f}, Std = {ref_std:.2f}, CV = {ref_cv:.2f}"
    msg2 = f"User Angular Speed: Mean = {user_mean:.2f}, Std = {user_std:.2f}, CV = {user_cv:.2f}"
    print(msg1)
    print(msg2)
    feedback = ""
    if user_cv > ref_cv + tolerance_cv:
        feedback = "Your motion appears unstable and too fast. Try to maintain a smoother, more controlled speed."
    elif user_cv < ref_cv - tolerance_cv:
        feedback = "Your motion is too slow. Try to maintain a faster speed."
    else:
        feedback = "Good stability! Your motion is consistent."
    print(feedback)
    return [msg1, msg2, feedback]

def compare_dtw_feedback(adjusted_dist_up, adjusted_dist_down, avg_adjusted_dist, rep_index):
    messages = []
    msg = f"\nUser Rep {rep_index} DTW Analysis:"
    print(msg)
    messages.append(msg)
    
    msg = f"Original DTW distance (Up phase):   {adjusted_dist_up + 4:.3f}"
    print(msg)
    messages.append(msg)
    msg = f"Original DTW distance (Down phase): {adjusted_dist_down + 9:.3f}"
    print(msg)
    messages.append(msg)
    msg = f"Adjusted DTW distance (Up phase):   {adjusted_dist_up:.3f}"
    print(msg)
    messages.append(msg)
    msg = f"Adjusted DTW distance (Down phase): {adjusted_dist_down:.3f}"
    print(msg)
    messages.append(msg)
    msg = f"Average Adjusted DTW distance:      {avg_adjusted_dist:.3f}"
    print(msg)
    messages.append(msg)
    
    if adjusted_dist_up < 4:
        msg = "Excellent! Your upwards motion closely matches the reference."
    else:
        msg = "Noticeable deviation in upwards motion. Try to align your form more closely."
    print(msg)
    messages.append(msg)
    if adjusted_dist_down < 9:
        msg = "Excellent! Your downwards motion closely matches the reference."
    else:
        msg = "Noticeable deviation in downwards motion. Try to align your form more closely."
    print(msg)
    messages.append(msg)
    if avg_adjusted_dist < 6:
        msg = "Good job! Your overall motion closely matches the reference."
    else:
        msg = "There are areas for improvement in your motion as your overall motion does not closely match the reference."
    print(msg)
    messages.append(msg)
...
def dtw_analysis(ref_file, user_rep_files, user_colors, local_offset=np.array([0.0, 0.0, 0.3])):
    ...
    for i, user_file in enumerate(user_rep_files):
        ...
        rep_feedback = compare_dtw_feedback(adjusted_dist_up, adjusted_dist_down, avg_adjusted_dist, i+1)
        dtw_feedback.extend(rep_feedback)
...
def stability_analysis(ref_file, user_rep_files):
    ...
    for i, user_file in enumerate(user_rep_files):
        ...
        feedback_list = compare_stability(compute_angular_speed(ref_file)[1], user_ang_speeds, tolerance_cv=0.2)
        for msg in feedback_list:
            print(msg)
            stability_feedback.append(msg)
    return stability_feedback
...
def main(folder="motion_files", duration=5, port='COM4'):
    global all_feedback
    all_feedback = []  # Reset the global feedback list
    ...
    dtw_feedback = dtw_analysis(ref_file, user_rep_files, user_colors)
    for msg in dtw_feedback:
        all_feedback.append(msg)
    ...
    for i, user_file in enumerate(user_rep_files):
        ...
        pos_feedback_title = f"\nXYZ Position Feedback for User Rep {i+1}:"
        print(pos_feedback_title)
        all_feedback.append(pos_feedback_title)
        for msg in feedback_messages:
            print(msg)
            all_feedback.append(msg)
    ...
    stability_feedback = stability_analysis(ref_file, user_rep_files)
    for msg in stability_feedback:
        all_feedback.append(msg)
    
    # At the end, summarise all the feedback via OpenAI
    final_summary = summarise_feedback(all_feedback)
